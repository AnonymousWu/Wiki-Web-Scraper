...
class Graph:
    ...
    def add_actor(self, new_actor):
        self.numVertices += 1
        self.actors[new_actor.actor_name] = new_actor

    def add_movie(self, new_movie):
        self.numVertices += 1
        self.movies[new_movie.movie_name] = new_movie
    ...
    def is_connected(self, v1, v2):
        if isinstance(v1, movie.Movie) and isinstance(v2, actor.Actor):
            if v2.actor_name in v1.actorList and v1.movie_name in v2.movieList:
                return True
        if isinstance(v1, actor.Actor) and isinstance(v2, movie.Movie):
            if v1.actor_name in v2.actorList and v2.movie_name in v1.movieList:
                return True
        return False
...
#  ---------------------------GRAPH QUERIES ------------------------------------- #
    ...
    def find_movie_gross(self, m):
        if not m or m not in self.movies.values():
            ...
            return 0
        return self.movies[m.movie_name].gross
    ...
    def list_actor_movies(self, a):
        if not a or a not in self.actors.values():
            ...
            return 0
        return self.actors[a.actor_name].movieList
    ...
    def list_movie_actors(self, m):
        if not m or m not in self.movies.values():
            logging.warning("cannot find movie")
            return 0
        return self.movies[m.movie_name].actorList
